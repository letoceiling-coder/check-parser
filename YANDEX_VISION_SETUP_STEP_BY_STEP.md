...
3. Заполните форму:
   ...
   - **Роли в каталоге:** Убедитесь, что роль **`ai.vision.user`** уже указана в поле "Роли в каталоге"
     - Если роли нет, нажмите **"+ Добавить роль"** и выберите `ai.vision.user`
     - Роль автоматически назначается для текущего каталога (который указан в URL: `b1g6d2bijseuj7sagqub`)

4. Нажмите **"Создать"**

5. После создания откройте созданный сервисный аккаунт (кликните на его имя)

## Шаг 2: Проверка роли (опционально, если не назначили при создании)

**Этот шаг можно пропустить, если вы назначили роль `ai.vision.user` при создании сервисного аккаунта.**

Если вы забыли назначить роль при создании, сделайте это сейчас:

1. В сервисном аккаунте перейдите на вкладку **"Права доступа"** (Access Rights)

2. Нажмите **"Назначить роли"** (Assign roles)

3. В выпадающем списке выберите роль: **`ai.vision.user`**
   - Если этой роли нет в списке, введите `ai.vision.user` вручную

4. Роль автоматически будет назначена для текущего каталога (который указан в URL)
...
## Шаг 4: Обновление .env на сервере (или Шаг 3, если роль уже назначена)
