...
return [
    ...
    |--------------------------------------------------------------------------
    */
    'detection_order' => ['sber', 'tinkoff', 'alfabank', 'ozonbank', 'vtb', 'raiffeisen', 'gazprombank', 'rosselkhozbank', 'otp', 'default'],
    ...
    */
    'banks' => [
        'sber' => [
            'name' => 'Сбербанк',
            'detect_keywords' => ['сбербанк', 'сбербанка', 'сбер банк', 'сбербанк онлайн', 'sberbank', 'мобильное приложение сбербанк'],
        ...
        'tinkoff' => [
            'name' => 'Тинькофф',
            'detect_keywords' => ['тинькофф', 'тинькофф банк', 'т-банк', 'т-банка', 'клиенту т-банка', 'tinkoff', 'tbank', 'номер кошелька'],
        ...
        'rosselkhozbank' => [
            'name' => 'Россельхозбанк',
            'detect_keywords' => ['россельхозбанк', 'росссельхозбанк', 'россельхозб', 'сельхозбанк', 'сельхозб', 'рсхб', 'rosselkhoz'],
            'sum_keys' => ['итого', 'сумма', 'исполнено', 'перевод'],
            'date_keys' => ['дата', 'время', 'операции'],
            'sum_regex' => [
                '/исполнено\s+(\d{1,3}(?:[\s\x{00A0}]\d{3})*(?:[.,]\d{2})?|\d+(?:[.,]\d{2})?)\s*[₽РрPpруб\.]?/ui',
                '/перевод[^\d]{0,30}(\d{1,3}(?:[\s\x{00A0}]\d{3})*(?:[.,]\d{2})?|\d+(?:[.,]\d{2})?)\s*[₽РрPpруб\.]?/ui',
            ],
            'date_regex' => [
                '/(\d{2})[\.\/](\d{2})[\.\/](\d{4}),\s+(\d{2}):(\d{2})/u',
                '/(\d{2})[\.\/](\d{2})[\.\/](\d{4})/u',
            ...
            'unique_regex' => [
                '/номер\s+карты\s+[^\d]*(\d{4})/ui',
            ],
        ],

        'otp' => [
            'name' => 'ОТП Банк',
            'detect_keywords' => ['ао "отп"', 'отп банк', 'otp bank', 'ооо "отп банк"'],
            'sum_keys' => ['сумма', 'итого', 'к переводу', 'к оплате'],
            'date_keys' => ['дата', 'от', 'выписки'],
            'sum_regex' => [
                '/сумма[^\d]{0,50}(\d{1,3}(?:[\s\x{00A0}]\d{3})*(?:[.,]\d{2})?|\d+(?:[.,]\d{2})?)\s*[₽РрPpруб\.]?/ui',
                '/к\s+переводу[^\d]{0,30}(\d{1,3}(?:[\s\x{00A0}]\d{3})*(?:[.,]\d{2})?|\d+(?:[.,]\d{2})?)/ui',
            ],
            'date_regex' => [
                '/№?\s*(\d{1,2})[\/](\d{2})[\/](\d{4})/u',
                '/(\d{2})[\.\/](\d{2})[\.\/](\d{4})/u',
    ...
    */
    'filename_hints' => [
        ...
        'ozonbank' => ['ozonbank', 'ozon_bank', 'ozon bank'],
