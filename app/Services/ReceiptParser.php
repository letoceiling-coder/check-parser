...
class ReceiptParser
{
    ...
    /** Regex числа (с пробелами в тысячах, дробная часть). (?!\d) не допускает "65 100" из "65 10000" */
    private const AMOUNT_REGEX = '(\d{1,3}(?:[\s\x{00A0}]\d{3})*(?!\d)(?:[.,]\d{2})?|\d{1,3}(?:\.\d{3})+(?:,\d{2})?|\d+(?:[.,]\d{2})?)';
